  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (let t = 0; t < tokens.length; t++) {
      ...
      for (let l = 1; l <= token.length; l++) {
        ...
        if (_prefixIndex[prefix].indexOf(i) === -1)
          _prefixIndex[prefix].push(i);
      }
    }
  ...
  if (words.length === 1) {
    ...
    if (prefixHits) {
      for (let i = 0; i < prefixHits.length; i++) {
        ...
        let display = item.display.toLowerCase();
        let band = display.startsWith(query)
          ? 400
          : item.tokens[0] && item.tokens[0].startsWith(query)
            ? 300
            : 200;
        scores[idx] = band;
      }
    ...
    for (let w = 0; w < words.length; w++) {
      if (words[w].length > 0)
        candidateSets.push(getCandidatesForWord(words[w]));
    }
